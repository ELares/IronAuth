...
The labels are checked the same way, and the kind is too: a site that emits a label this page
does not list, or omits one it does, or emits as a gauge what this page calls a counter, fails.
...
it reached 11 of the 41 metrics below while this page told a dashboard author it reached all of
them. A `tenant` label added to any of the other 30 emit sites left every test green. The scan
...
server crate and is not limited to it: of the 41 entries, 4 are emitted from `ironauth-server`,
28 from `ironauth-oidc`, 7 from the binary crate and 2 from `ironauth-fetch`.
