...
- the **linked user** must hold the mapped permission -- `{resource.type}.{action.name}`, the same
  pure join the other two subject types use, resolved through the same `effective_permissions`
  the token claims are built from. A second copy of that walk would be a second set of answers.
...
organization**, or to none, is denied -- not reported missing: a PDP answers decisions, and
