...
Ending the session ends the family's ability to bootstrap, **by any route**. The device-secret row hangs off the **sign-in**, not off the app that asked for it, and redemption **joins `sessions` and requires it live**. So an operator revoking in the console, a bulk revoke, a password change, a risk decision, global token revocation and plain expiry all sever the set without needing to know this table exists.
...
- **`device_sso` is not gated per client.** Arming the flag lets any client in any environment of the deployment ask for a device secret. What bounds the damage is that redemption needs the matching ID token and a live session; a graduation should still make it a registration property.
