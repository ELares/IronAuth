...
| Information disclosure | The mapping endpoints used to enumerate a sibling organization's roles or a sibling environment's capability names | Uniform not-found across every shape, asserted BYTE for byte against a reference that is reachable and simply not attached: a soft-deleted role, a sibling organization's role, a sibling environment's permission, a malformed id, an id carrying another resource type's prefix, and a blank segment, on the list, the attach, and the detach. The reference fixtures name rows that genuinely EXIST elsewhere, so a probe cannot pass merely because nothing was there |
