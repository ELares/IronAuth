...
may carry permission claims. Nothing in this PR emits a claim, so the flag is
inert today; the surface is nonetheless the place an operator widens what an
entire audience's tokens will assert once issue #98's activation lands.
...
| Elevation | The `opaque` plus opted-in combination reached by a path this API does not guard | Stated as a limit rather than controlled, because it is real. `resource_servers` is a PROMOTABLE resource type, and a config promotion writes `token_format` and `permission_claims_enabled` from one source snapshot in one statement with no management handler in the path, so a promoted snapshot CAN land that combination. Migration 0094 deliberately ships no CHECK constraint against it: a CHECK would turn a whole-environment promotion into an opaque 500 over a setting whose only consequence is that a claim is not emitted. The combination is INERT rather than unsafe, because an opaque access token carries no claims at all and the flag has nowhere to apply. Nothing re-checks the format after a promotion, and a later read reports exactly what is stored rather than a corrected value |
| Elevation | The opt-in widening what an outstanding access token asserts | Not reachable in this PR, because nothing reads the column on the mint path yet. When the activation PR wires it, the same KNOWN GAP and the same bound the sections above record will apply: a change takes effect at the NEXT token issuance and revokes no token already issued, so the exposure is at most one access-token lifetime, kept to that bound because the refresh grant re-resolves rather than replaying |
