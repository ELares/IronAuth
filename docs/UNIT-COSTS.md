...
|---|---|
| **one IronCache `GET` hit, 512-byte value, loopback TCP** | **30 us** |
...
protocol; nothing here measures one.
...
measured. An IronCache `GET` hit costs **30 us**: about what a bare Postgres round trip costs,
because both are a loopback round trip, and the difference is what the server does on top of it.
IronCache does a few microseconds of work; a scoped read does a hundred and forty.

So the two tiers answer the question differently, and the table is the whole argument:

| standing in front of one scoped read (166 us) | hit costs | saves |
|---|---|---|
| the Postgres tier (`PgHotState`) | 145 us | 13 per cent |
| an attached IronCache | 30 us | **82 per cent** |

That is the number every wiring decision above turns on, and it says the seam is worth attaching
an accelerator to. Standing in front of a resolved tenant config, which is three scoped
transactions, the same hit saves about 94 per cent.

TWO THINGS IT DOES NOT SAY. The measurement is a protocol round trip on one connection, so it is
a FLOOR for the `redis`-crate path `ironauth-hot` actually uses, which adds its own encoding and
connection handling. And it is a HIT: a miss costs this plus the read it failed to avoid plus the
write-back, so the value of attaching one is a function of hit rate, which is a property of a
deployment rather than of this software.
