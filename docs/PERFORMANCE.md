...
What that proves has changed, and this paragraph has now been wrong in both directions, so it is
worth being exact. It first claimed readiness came after "the first database round trip", which
was false: the probe was a bare `TcpStream::connect` whose own comment said "no bytes are
exchanged and no database protocol is spoken", and a review confirmed `/readyz` answering `ready`
against a database holding zero tables.

Issue #149 replaced that. `ReadinessProbe` now asks a `DatabaseProbe` for a real query on the
serving pool, so the first `ready` does come after a completed round trip, and a pod whose schema
is unmigrated answers 503 instead. THE FIGURES BELOW PREDATE THAT and were measured against the
socket check, so they are process start to listener-up rather than to first-query. They are a
floor for the current build, and re-measuring them is what the release benchmark lane is for.

So the numbers below are process start to listener-up, with the Postgres address proven
TCP-reachable. That is
