...
2. **The grants behind its outstanding tokens are revoked.** An access token derives its active
   ...
   inactive at introspection, at UserInfo, and at the agent vault exchange, and any refresh
   chain stops.
...
At the default `access_token_ttl_secs: 300` that is a five-minute worst case between an operator
...
so a per-organization SIEM stream delivers them and a correlation by person finds them. The
