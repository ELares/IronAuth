...
| `crates/ironauth-oidc/src/transaction_tokens.rs` (1, NOT counted by the generator) | A `txn_token` transaction token (issue #133, PROTOTYPE) | The `usr_` subject the exchange's REVALIDATED subject token carried | **The exchange's own `fence_principal`**, which runs before this branch is reached and refuses a blocked, disabled or deleted subject. Nothing after that: the token opens no grant, is not introspectable, and CANNOT BE REVOKED. Its five-minute expiry is the whole remaining mitigation, which is why the lifetime is clamped at the mint rather than configured |
