...
- It sees only calls THROUGH `crate::tokens`. A module that reaches
  `ironauth_jose::sign_jws` directly mints a user-bound token the generator cannot see, and one
  does: `transaction_tokens.rs` (issue #133, PROTOTYPE) signs a `txn_token` carrying a `usr_`
  subject taken from the exchange's revalidated subject token. It is listed in the table below
  by hand for that reason, and the answer the registry exists to force is unusually worth
  writing down here: **nobody can revoke it.** It opens no grant, is not introspectable, and
  lives at most five minutes; the fence is the exchange's own `fence_principal`, which refuses
  a blocked, disabled or deleted subject BEFORE the mint, and the expiry is everything after
  that. A graduation that made transaction tokens revocable would move this row.
...
| `crates/ironauth-oidc/src/ciba_grant.rs` | The CIBA grant's ID + access token | The `usr_` id the backchannel request named, resolved from the `login_hint` to a live `users` row by `ciba.rs` before the request is stored | **Directly, and UNCONDITIONALLY.** A third path of the shape issue #241 was filed for, and the one where BOTH existing mechanisms miss rather than one. The session cascade cannot reach it: `cascade_user_sessions_ended` updates `sessions`, `client_sessions` and `refresh_families`, and a backchannel request records no session at all (`decide` inserts its grant with `session_ref = NULL`). Grant revocation cannot either: `verify_redemption_grant`'s only liveness term is `revoked_at IS NULL`, and `cascade_families_for_subject` revokes a grant only under `hard_kill` and only when a `refresh_families` row names it. This grant issues no refresh token, so it has no family, so even `delete_user` leaves the approval live. `issue_ciba_tokens` therefore calls `ensure_subject_can_authenticate` before `mint_ciba_tokens`. Unconditional rather than shape-gated, for the reason `resolve_device_sid` gives on its session-less branch: the subject is a `usr_` id by construction and nothing operator-authored reaches here. MEASURED by `a_fenced_user_cannot_redeem_an_approved_backchannel_request` in `crates/ironauth-oidc/tests/ciba_grant.rs`, which drives every non-authenticatable `UserState` (Blocked and Disabled by transition, PendingVerification and Waitlisted by seeding, since nothing transitions INTO those two) plus a soft delete, against an ACTIVE control that mints. Before the fence existed, a Blocked and a soft-deleted user each answered 200 with a live ID token and access token. What makes this mint distinctive is how little there is to take back afterwards: the ID token carries no `sid`, so back-channel logout has nothing to target, and under the default `at_jwt` access-token format nothing revokes the access token before its expiry either (under `opaque` it is revocable through its grant) |
| `crates/ironauth-oidc/src/transaction_tokens.rs` (1, NOT counted by the generator) | A `txn_token` transaction token (issue #133, PROTOTYPE) | **Whatever the subject token carried**, exactly as the sibling row above: a `usr_` id from an interactive grant, or an `sva_` service account when the subject token came from client credentials | **The exchange's own `fence_principal`**, which runs before this branch is reached. It refuses a blocked, disabled or deleted USER and returns `Ok(())` unconditionally for a workload principal, which bears no lifecycle -- so for a service-account subject there is no lifecycle fence, here or on the ordinary path. Nothing after that either way: the token opens no grant, is not introspectable, and CANNOT BE REVOKED. Its five-minute expiry is the whole remaining mitigation, which is why the lifetime is clamped at the mint rather than configured |
