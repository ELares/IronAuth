...
and column scoped least privilege grants (the #31 lesson):
...
- **Schema migration (#53).** The wrapped run reconciles a completed trait migration
  job: a migrated identity is imported + consistent + backfilled (its
  `traits_schema_version` now stamps the target); a failed identity (from the job's
  per record failure report) is failed + inconsistent, which the CONSISTENCY
  invariant then refuses to complete over.
...
- A real schema migration job with failed identities, wrapped in the machine, is
  blocked, and the violation view names EXACTLY the identities the job's failure
  report listed.
...
paths adopt it immediately (both adapters exist) and lets operators opt a run into
the gate without forcing every future migration author through it before the ergonomics
are proven.

Before promoting it to the DEFAULT for all long running mutations, three gaps should
...
(2) `try_complete` re-evaluates under the run row's lock, but a concurrent ingest
committing between evaluation and the state write is not fenced by that lock alone;
the mandatory default should lock the record set (or serialize ingest against
completion). (3) The tenant move adapter (M5) should be built and its consistency
predicate (profile AND credentials landed) exercised against a real move before the
machine is claimed as the universal lifecycle. Until then, opt in captures the
safety win without over committing an exploratory surface.
