...
| Deployment tooling | Helm, a Debian package with systemd unit, static musl build/release lanes, database migration commands, preflight `doctor`, and storage recovery/rekey tools. The checked-in general Compose stack has build/configuration/healthcheck limitations; see [container setup](OPERATIONS.md#containers-and-the-checked-in-compose-files). |
...
| Integration tooling | TypeScript verification/protocol utilities, Node BFF middleware, framework quickstarts, generated management SDKs, CLI tools, MCP library implementations, and a Terraform provider. Consult each artifact's scope and compatibility before adopting it. |
