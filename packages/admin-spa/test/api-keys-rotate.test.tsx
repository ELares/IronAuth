...
//
// The organization API keys panel's ROTATE control (issue #99, criterion 6).
//
...
const NEW_KEY = "ira_ak_akey_CCCC~DDDD";

describe("api keys rotate control, display once", () => {
  beforeEach(() => {
    resetScope();
  });
  ...
  it("rotates in ONE request and shows the replacement once", async () => {
    let rotated = false;
    const { root, calls } = await loadPanel((call) => {
      ...
      return json({
        items: rotated
          ...
          : [{ id: "akey_old", display_name: "ci" }],
      });
    });

    button(root, "Rotate").click();
    await flush();
    const confirm = Array.from(root.querySelectorAll("button")).find(
      (element) => element.textContent === "Confirm rotate",
    );
    confirm?.click();
    await flush();

    // Non-vacuity: the rotate actually fired.
    expect(rotated).toBe(true);
    // ONE request, not create-then-revoke. Two POSTs here would mean the panel had
    // reintroduced the window where both keys are live, which is the failure the
    // single store transaction exists to prevent.
    expect(calls.filter((call) => call.method === "POST")).toHaveLength(1);
    expect(calls.filter((call) => call.method === "DELETE")).toHaveLength(0);
    // The replacement is shown, and the now-revoked original is still listed so the
    // rotation is legible.
    expect(root.textContent).toContain(NEW_KEY);
    expect(root.textContent).toContain("Revoked");
  });

  it("shows no secret when a rotate replay answers without a key", async () => {
    const { root } = await loadPanel((call) =>
      call.method === "POST"
        ...
        : json({ items: [{ id: "akey_old", display_name: "ci" }] }),
    );
    button(root, "Rotate").click();
    await flush();
    Array.from(root.querySelectorAll("button"))
      .find((element) => element.textContent === "Confirm rotate")
      ?.click();
    await flush();
