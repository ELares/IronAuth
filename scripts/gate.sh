...
# THE CORPUS THAT EMBEDS THE GENERATED DOCS, placed after the lanes that WRITE them.
#
# docs/llms-full.txt inlines the verbatim text of every published docs/*.md, and several of
# those are themselves generated by lanes above -- COMPATIBILITY.md, CONFIG.md and EVENTS.md
# among them. Regenerating any of them without regenerating the corpus leaves CI red, and red
# EARLIER than the check that would explain it: the invariants job runs the llms step before
# the compat step and aborts there. Not hypothetical -- a PR fixing the compat freshness
# failure moved that job's failure earlier and reported the fix as landed, because this line
# did not exist and nothing runnable locally could see it.
#
# "Among them" and not a count. An earlier version of this comment said TWO, naming only
# COMPATIBILITY.md and CONFIG.md, and was wrong the moment it was written: event-catalog.sh
# writes EVENTS.md above too. A census in prose beside a set that grows is a number waiting
# to be wrong, which is the defect this whole lane exists to catch.
#
# Position: after every lane that WRITES a published doc. The sdk-contract and
# events-vs-webhooks lanes run later but pass --check, which does not touch the tree
# (measured, not assumed), so they cannot stale the corpus mid-run. If either ever writes in
