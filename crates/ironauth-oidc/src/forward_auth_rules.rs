...
pub fn layered_limiter_from_config(
    ...
    // PER-CLIENT AND PER-USER ARE MAPPED HERE because the SHARED builder serves surfaces
    // that resolve them: the authorization path keys per-client on the verified client
    // identifier, and the token endpoint's grants key per-user on the subject resolved
    // from the code binding or the refresh family. A surface that resolves neither (the
    // forward-auth check) still receives the layers in its limits, and a request without
    // a key for a CONFIGURED layer is reported in the outcome's `unenforced` census
