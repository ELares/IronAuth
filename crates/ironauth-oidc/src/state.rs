...
/// The web origin (`scheme://host[:port]`) of a URL, or [`None`] when it has no
/// `scheme://authority`. The path, query, and fragment are dropped, so a base URL
/// that carries a path (`https://host/base`) still yields the bare origin an
/// `Origin` header would (`https://host`). Used to derive this deployment's own
/// origin from `issuer_base` for the CSRF Origin allowlist (issue #196).
...
mod tests {
    ...
    fn origin_of_keeps_scheme_and_authority_and_drops_the_path() {
        ...
        // A base URL with a path or port still yields the bare origin.
        assert_eq!(
