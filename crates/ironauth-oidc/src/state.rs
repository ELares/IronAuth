...
impl OidcState {
    ...
    fn build(
        ...
        Self {
            ...
            ssf_max_subjects_per_stream: ironauth_config::SsfConfig::default()
                .max_subjects_per_stream,
            fedcm_enabled: false,
    ...
    pub fn with_ssf(mut self, config: &ironauth_config::SsfConfig) -> Self {
        ...
        self.ssf_max_subjects_per_stream = config.max_subjects_per_stream;
        self
    }
    ...
    #[must_use]
    pub fn ssf_max_subjects_per_stream(&self) -> u32 {
        self.ssf_max_subjects_per_stream
    }
    ...
    pub(crate) async fn enforce_request_quota(
        ...
        if let Some(limiter) = self.layered_limiter.as_ref() {
            let identity = ironauth_quota::layered::RequestIdentity {
                ...
                // THE VERIFIED SUBJECT, when the handler has resolved one (issue #150
                // criterion 1): the token endpoint's grants key it on the subject from the
                // code binding or the refresh family.
    ...
    pub(crate) async fn verify_access_token(
        ...
        // The keys published at `now` and RETAINED in this environment's JWKS are
        // exactly those a currently-valid token could have been signed by (the
        // rotation retention rule); a token's `kid` only selects among these, never
        // introduces one (issue #9's verify path). Filtering through the policy's
        // JWKS-retention predicate makes the allowlist EXACTLY the algorithms the
        // JWKS publishes, so verify never accepts an algorithm this environment does
        // not advertise (issue #204).
        ...
        // The allowlist is exactly the algorithms those retained keys sign with.
    ...
    pub(crate) async fn verify_logout_hint(
        ...
        let entry = self.issuer_entry(scope).await.ok_or(())?;
        // Filter through the JWKS-retention predicate exactly as the access-token
        // path does, so a logout hint is trusted against EXACTLY the algorithms this
        // environment publishes (issue #204).
        let keys = published_verify_keys(&entry, self.now());
        if keys.is_empty() {
            return Err(());
        }
...
/// The keys this environment's verify path trusts at `now`: the published signing
/// keys, filtered to those the policy RETAINS in the JWKS.
///
/// This mirrors [`ironauth_jose::KeySet::published_jwks`]'s predicate EXACTLY
/// (`SigningPolicy::retains_in_jwks`, NOT `permits`), so verify trusts exactly the
/// key set the JWKS publishes: a legitimately-retained `RS256` downgrade key still
/// verifies (the zero-friction downgrade covenant, `retains_in_jwks(RS256)` is
/// always true), while a non-permitted non-`RS256` retention-window key is rejected
/// because the JWKS does not advertise it (issue #204).
fn published_verify_keys(entry: &IssuerEntry, now: SystemTime) -> Vec<&SigningKey> {
    let policy = entry.policy();
    entry
        .keyset()
        .published_signing_keys(now)
        .into_iter()
        .filter(|key| policy.retains_in_jwks(key.algorithm()))
        .collect()
}
...
mod tests {
    ...
    use ironauth_store::{EnvironmentType, GuardrailSet};

    use super::{IssuerEntry, origin_of, published_verify_keys};
    use crate::subject::PairwiseSalt;

    /// A committed ECDSA P-256 PKCS#8 key (the ironauth-oidc test fixture), for an
    /// ES256 key present in a key set but excluded by a policy that does not permit
    /// ES256.
    const ES256_PKCS8: &[u8] = &[
        ...
        0x6b, 0x02, 0x01, 0x01, 0x04, 0x20, 0xfc, 0x76, 0xdf, 0x7c, 0x3d, 0x9f, 0xef, 0x33, 0x39,
        0x20, 0x6f, 0x02, 0xe9, 0xec, 0xb3, 0x30, 0x0b, 0xcd, 0x3b, 0x01, 0xf4, 0x09, 0x91, 0x10,
        0x23, 0x75, 0x80, 0xd2, 0xda, 0x1b, 0x3e, 0xf9, 0xa1, 0x44, 0x03, 0x42, 0x00, 0x04, 0x85,
        0xd1, 0x32, 0xad, 0x68, 0xc7, 0x5b, 0x7e, 0xd4, 0x5c, 0x7e, 0xef, 0x46, 0x5e, 0x98, 0xa3,
        0x30, 0xb6, 0x71, 0x4b, 0x9a, 0xfb, 0x29, 0xc9, 0xbd, 0xdf, 0xaa, 0x2a, 0xe6, 0xf8, 0xdf,
        0x63, 0xc4, 0x97, 0x49, 0x4b, 0x76, 0xcc, 0x05, 0xbe, 0xdc, 0x5f, 0xb9, 0xb8, 0xe7, 0x1c,
        0x9c, 0x86, 0x4e, 0x47, 0xde, 0x6f, 0xf4, 0x08, 0xf2, 0x34, 0x12, 0x9d, 0xb0, 0x02, 0x94,
        0xe0, 0xc7, 0x4d,
    ];

    /// Build a three-algorithm key set (`EdDSA`, ES256, RS256) whose policy permits
    /// ONLY `EdDSA`, so ES256 is present-but-excluded and RS256 is
    /// present-but-retained by the downgrade covenant. Returns the entry plus each
    /// key's kid.
    fn divergent_entry() -> (IssuerEntry, String, String, String) {
        let ed = SigningKey::ed25519_from_seed(Some("ed-kid".to_owned()), &[0x11; 32])
            .expect("ed25519 key");
        let es = SigningKey::ecdsa_p256_from_pkcs8(Some("es-kid".to_owned()), ES256_PKCS8)
            .expect("es256 key");
        let rsa = SigningKey::rsa_from_pkcs1_der(
            Some("rsa-kid".to_owned()),
            JwsAlgorithm::Rs256,
            RSA_PKCS1,
        )
        .expect("rsa key");
        let (permitted_kid, excluded_kid, downgrade_kid) = (
            ed.kid().expect("ed kid").to_owned(),
            es.kid().expect("es kid").to_owned(),
            rsa.kid().expect("rsa kid").to_owned(),
        );
        let mut keyset = KeySet::bootstrap(ed, SystemTime::UNIX_EPOCH);
        keyset.add(es, SystemTime::UNIX_EPOCH);
        keyset.add(rsa, SystemTime::UNIX_EPOCH);
        // Policy permits ONLY EdDSA: ES256 is present but not permitted, RS256 is
        // present, not permitted, yet retained by the downgrade covenant.
        let policy = SigningPolicy::new(vec![JwsAlgorithm::EdDsa]).expect("policy");
        let entry = IssuerEntry::new(
            keyset,
            policy,
            PairwiseSalt::new(Vec::new()),
            GuardrailSet::for_kind(EnvironmentType::Dev),
        );
        (entry, permitted_kid, excluded_kid, downgrade_kid)
    }

    #[test]
    fn verify_trusts_exactly_the_published_jwks() {
        // Issue #204 item 4: the verify allowlist is filtered through the policy's
        // JWKS-retention predicate, so verify trusts EXACTLY the keys the JWKS
        // publishes. A permitted key (EdDSA) and the RS256 downgrade key are retained;
        // a present-but-non-permitted non-RS256 key (ES256) is dropped.
        let (entry, permitted_kid, excluded_kid, downgrade_kid) = divergent_entry();
        let now = SystemTime::UNIX_EPOCH;

        // The published JWKS: EdDSA (permitted) and RS256 (covenant), NOT ES256.
        let mut jwks_kids = entry.keyset().published_kids(now, entry.policy());
        jwks_kids.sort();
        assert_eq!(
            jwks_kids,
            vec![permitted_kid.clone(), downgrade_kid.clone()],
            "the JWKS publishes the permitted key and the RS256 downgrade key only"
        );

        // The verify path trusts EXACTLY that set (verify == JWKS).
        let mut verify_kids: Vec<String> = published_verify_keys(&entry, now)
            .iter()
            .filter_map(|key| key.kid().map(str::to_owned))
            .collect();
        verify_kids.sort();
        assert_eq!(
            verify_kids, jwks_kids,
            "the verify allowlist matches the published JWKS exactly"
        );

        // The excluded ES256 key is trusted by NEITHER (the fix), while the RS256
        // downgrade key stays trusted (the covenant).
        assert!(
            !verify_kids.contains(&excluded_kid),
            "a present-but-non-published algorithm is not accepted at verify"
        );
        assert!(
            verify_kids.contains(&downgrade_kid),
            "a retained RS256 downgrade key still verifies"
        );
    }

    /// A committed RSA-2048 PKCS#1 private key (DER), the ironauth-oidc test fixture,
    /// for an RS256 key present in the key set (retained by the downgrade covenant).
    const RSA_PKCS1: &[u8] = &[
        ,
    ];
