...
//!
//! Engine convergence (issue #92, CLOSED by PR 8e): the mint family (login, registration,
//! recovery) is driven through the ONE compiled-table engine ([`orchestration::drive_via_table`]),
//! byte identically to the imperative drivers it replaced (now deleted). Federation and Consent
//! remain imperative in this module ([`drive_federation`] / [`drive_consent`]) as the two
//! documented permanent residuals (an owner-scope decision).
//!
...
// A flat per-journey dispatch match: the mint family (login, registration, recovery) shares one
// table-engine arm; Federation and Consent are the two imperative residuals.
pub async fn drive(
    state: &OidcState,
    scope: Scope,
    flow_id: &FlowId,
    ...
    match journey {
        // The mint family (issue #92, PR 8b/8c/8d, closed by PR 8e): login, registration, and
        // recovery all drive through the ONE compiled-table engine, byte identically to the retired
        // imperative drivers (deleted in PR 8e). The bound `journey` selects the embedded artifact,
        // so this single arm is exactly what the three literal per-journey arms passed. The login
        // flip subsumes the in-flow MFA challenge / enroll and progressive-profiling holds, because
        // they are substates of the login state machine (not separate journeys).
        Journey::Login | Journey::Registration | Journey::Recovery => {
        ...
        // Federation and Consent stay imperative in this module as the two documented permanent
        // residuals (an owner-scope decision): each is a redirect or grant step with no mint, not a
        // login state machine, so neither converges onto the table engine.
