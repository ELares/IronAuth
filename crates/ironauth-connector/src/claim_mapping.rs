...
///
/// # Traversal wins, and a LITERAL key is the fallback -- SAML is why either matters
///
...
/// ADFS and Entra emit for an email address. Splitting on `.` and nothing else looks up the key
...
/// would be a sentence with no reachable path behind it.
///
/// TRAVERSAL IS TRIED FIRST, AND THAT ORDER IS THE SECURITY PROPERTY. An earlier version tried
/// the literal key first, which let the UPSTREAM shadow a mapping author's path: an identity
/// provider emitting a claim named literally `emails.0` would have displaced the shipped
/// `emails.0` traversal, and the upstream chooses that key, not the operator. Traversal-first
/// means no document can take a path away from the mapping that asked for it.
///
/// THE FALLBACK IS OVER PREFIXES, so both halves work. When traversal fails, the longest prefix
/// that is a literal key is tried and whatever remains is read as traversal from there. That
/// reaches the whole dotted name AND an index inside it: `http://.../claims/groups.0` finds the
/// key, then takes element zero -- which is what a multi-valued SAML attribute needs and what a
/// whole-key-only lookup could not do.
fn resolve_path<'a>(root: &'a Map<String, Value>, path: &str) -> Option<&'a Value> {
    if let Some(value) = traverse(root, path) {
        return Some(value);
    }
    // THE WHOLE PATH AS A KEY, which is the ordinary SAML case: the mapping names an attribute
    // and wants the attribute.
    if let Some(value) = root.get(path) {
        return Some(value);
    }
    // THEN THE LONGEST LITERAL PREFIX, with the remainder read as traversal from there. Longest
    // first so a provider sending both `a.b` and `a.b.c` resolves the more specific key, which
    // is the same rule traversal-before-literal follows: the reading that used more of the path
    // wins.
    let mut split = path.len();
    while let Some(cut) = path[..split].rfind('.') {
        split = cut;
        if let Some(value) = root.get(&path[..cut]) {
            return descend(value, &path[cut + 1..]);
        }
    }
    None
}

/// The dotted-path traversal, with no literal-key fallback.
fn traverse<'a>(root: &'a Map<String, Value>, path: &str) -> Option<&'a Value> {
    let mut segments = path.split('.');
    let first = segments.next()?;
    descend_segments(root.get(first)?, segments)
}

/// Read `rest`, a dotted remainder that may be empty, from inside `value`.
fn descend<'a>(value: &'a Value, rest: &str) -> Option<&'a Value> {
    if rest.is_empty() {
        return Some(value);
    }
    descend_segments(value, rest.split('.'))
}

/// Walk `segments` from `current`, into an object by key and into an array by index.
fn descend_segments<'a, 'b>(
    current: &'a Value,
...
mod tests {
    ...
    fn traversal_still_works_and_the_literal_key_wins_where_both_could() {
        ...
        // WHERE BOTH COULD RESOLVE, TRAVERSAL WINS, AND THAT ORDER IS THE SECURITY PROPERTY.
        // An earlier version preferred the literal key, which let the UPSTREAM shadow a mapping
        // author's path: an identity provider that emits a claim named literally `emails.0`
        // would displace the shipped `emails.0` traversal, and the upstream chooses that key,
        // not the operator. No document can take a path away from the mapping that asked for it.
        ...
        assert_eq!(
            ...
            Some(&Value::String("traversed".to_owned())),
            "an upstream-chosen literal key displaced a mapping author's traversal"
        );

        // AND THE SHIPPED PATH IS THE ONE THAT MATTERS: `emails.0` is a real preset path, so a
        // provider sending a claim of that name must not capture it.
        let shadowed = json!({
            "emails.0": "attacker@evil.example",
            "emails": ["ada@globex.example"],
        });
        let Value::Object(root) = &shadowed else {
            panic!("an object")
        };
        assert_eq!(
            resolve_path(root, "emails.0"),
            Some(&Value::String("ada@globex.example".to_owned())),
            "an upstream claim named `emails.0` captured the traversal a preset depends on"
        );
    }

    #[test]
    fn a_value_inside_a_dotted_attribute_is_addressable_through_it() {
        // A WHOLE-KEY-ONLY FALLBACK REACHES THE ATTRIBUTE AND NOTHING INSIDE IT, and a SAML
        // attribute is MULTI-VALUED: `groups` is the one every deployment maps to roles. So the
        // fallback tries the longest literal PREFIX and reads the remainder as traversal, which
        // is what makes `<name>.0` work.
        const GROUPS: &str = "http://schemas.microsoft.com/ws/2008/06/identity/claims/groups";
        let claims = json!({ GROUPS: ["engineering", "oncall"] });
        let Value::Object(root) = &claims else {
            panic!("an object")
        };
        assert_eq!(
            resolve_path(root, GROUPS),
            Some(&json!(["engineering", "oncall"])),
            "the attribute itself was not addressable"
        );
        assert_eq!(
            resolve_path(root, &format!("{GROUPS}.1")),
            Some(&Value::String("oncall".to_owned())),
            "a value inside a dotted attribute name was not addressable"
        );
        assert_eq!(resolve_path(root, &format!("{GROUPS}.9")), None);

        // THE LONGEST PREFIX WINS. This needs a path whose WHOLE form is NOT a key -- otherwise
        // the whole-key fallback answers first and longest-vs-shortest never runs, which is why
        // an earlier version of this case could not tell them apart.
        //
        // Two literal keys, one a prefix of the other, and a path one segment past both: the
        // longest reads `urn:a.b.c` then `d`; the shortest reads `urn:a.b` then `c` then `d`.
        // Both resolve, to different values, so only the precedence decides.
        let nested_keys = json!({
            "urn:a.b": { "c": { "d": "shorter" } },
            "urn:a.b.c": { "d": "longer" },
        });
        let Value::Object(root) = &nested_keys else {
            panic!("an object")
        };
        assert_eq!(
            resolve_path(root, "urn:a.b.c.d"),
            Some(&Value::String("longer".to_owned())),
            "the shorter literal prefix won, so the more specific key an operator configured was \
             passed over"
        );

        // AND INTO AN OBJECT under a dotted key, which is the same rule one level further.
        let nested = json!({ "urn:x.y": { "inner": "value" } });
        let Value::Object(root) = &nested else {
            panic!("an object")
        };
        assert_eq!(
            resolve_path(root, "urn:x.y.inner"),
