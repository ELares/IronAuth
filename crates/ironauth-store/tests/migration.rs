...
const CHAIN_SUBJECTS: &str = "isolation, audit log, \
     ...
     SAML connections, SAML connection certificates, \
     ...
     client tls client auth, cnf x5t s256, use mtls endpoint aliases, fapi hardened";
...
async fn every_scim_table_carries_its_isolation_structurally() {
    ...
    for (table, policy) in [
        ...
        ("saml_connections", "saml_connections_scope"),
        (
            "saml_connection_certificates",
            "saml_connection_certificates_scope",
        ),
        (
            "saml_outstanding_requests",
            "saml_outstanding_requests_scope",
        ),
        ("saml_assertion_replay", "saml_assertion_replay_scope"),
    ] {
        assert!(
            rls_enabled_and_forced(pool, table).await,
