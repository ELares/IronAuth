...
impl EnvironmentRepo<'_> {
    ...
    pub async fn get(&self, id: &EnvironmentId) -> Result<EnvironmentRecord, StoreError> {
        let row = sqlx::query(
            "SELECT id, tenant_id, display_name, region, \
             (EXTRACT(EPOCH FROM created_at) * 1000000)::bigint AS created_us \
             FROM environments \
...
impl ActingEnvironmentRepo<'_> {
    ...
    pub async fn create(
        ...
        // The per-environment residency pin (issue #46) is bound here on create and
        // never updated after (the control grant is column-scoped to exclude it), so
        // it is immutable for the environment's life.
