...
async fn a_keyed_writes_replay_survives_the_environments_deletion() {
    ...
    for (index, (label, path, body)) in writes.iter().enumerate() {
        ...
        let (status, _, response) = h.post(path, key, body).await;
        // A REPLAY RETURNS THE STORED ANSWER, so the expectation is the status the route gave
        // the FIRST time rather than the shape of its payload. Those were the same question
        // while every 200-first route also carried a secret, and the access-review export is
        // the first that does not: it answers 200 with a file and carries nothing secret, so
        // keying on `SECRET_BEARING_WRITES` alone demanded a 201 it never gave.
        let expected =
            if SECRET_BEARING_WRITES.contains(label) || FIRST_RESPONSE_IS_OK.contains(label) {
                StatusCode::OK
            } else {
                StatusCode::CREATED
            };
