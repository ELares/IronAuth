...
//!
//! 2. EVERY BOUND IS RE-MEASURED on the accepted document, against the SAME limits the parse
//!    used, and against the effective depth rather than the ceiling. A bound checked in the
//!    wrong place, or against the wrong number, fails here.
//!
...
fuzz_target!(|data: &[u8]| {
    // Sized so a 4096-byte input can cross every one of them.
    ...
    while let Some((element, depth)) = pending.pop() {
        ...
        widest = widest.max(element.children().len());
